...
In the next experiment (whih would be the 3rd space for me instead of 2nd), I would like to follow this question through since it opens up another path for me: if the language modle is smart enough to figure out the spatial relationship within the inputs, then it can just conecct itself to the image processing part, without me even finding and attaching another model that's dedicated to it. 
